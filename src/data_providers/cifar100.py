...
class Cifar100(DataProvider):
    def __init__(self):
        # origin image size is 32 * 32, cropped to 28 * 28
        super().__init__(image_size=28, num_classes=100, train_size=50000, test_size=10000)
        self._data_path = '../data/cifar-100-binary'
        self._num_preprocess_threads = 1

    def _get_input(self, phase, batch_size, is_training):
        if phase == 'test':
            filename = [os.path.join(self._data_path, 'test.bin')]
        else:
            filename = [os.path.join(self._data_path, 'train.bin')]
        ...
        images, labels = self._read_cifar100(filename_queue)
    ...
    def _read_cifar100(self, filename_queue):
        image_size = 32
        num_channels = 3
        ...
        fine_label.set_shape([1])
        return images, fine_label
